    ...
    <footer className="bg-background border-t border-border">
      <div className="container mx-auto px-4 py-12">
        <div className="grid md:grid-cols-3 gap-10 mb-10">
          {/* Brand */}
          <div>
            <img src={logo} alt="Jay BMW - Always Fresh" className="h-20 w-auto mb-4" />
            <p className="text-muted-foreground mb-6 max-w-xs">
              Serving authentic Himalayan flavors with love. Your favorite destination for butter momos and soul-warming soups.
            </p>
            {/* Social Links */}
        ...
        <div className="border-t border-border pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-sm text-muted-foreground text-center sm:text-left">
            © {new Date().getFullYear()} JH Butter Momos & Soup Bar. All rights reserved.
          </p>
          <p className="text-sm text-muted-foreground flex items-center gap-1">
            Made with <Heart className="w-4 h-4 text-primary fill-primary" /> in Uttarkashi
          </p>
        </div>
      </div>
    </footer>
